...
def search():
    ...
    with sr.Microphone() as source1:
        ...
        audio1=r.listen(source1,timeout=20,phrase_time_limit=8)
        try:
            print("recognising....")
            text=r.recognize_google(audio1,language='en-in')
            print(text.lower())
        except sr.UnknownValueError:
            #speak("sorry i didnt reach you please say that again")
            print("trouble with the microphone")
            #quit()   
            return "None"
        except sr.RequestError:
            engine.say('please turn on your internet connection')
            engine.runAndWait()
            quit()
        #return None
    return text
